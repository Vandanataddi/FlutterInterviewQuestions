...
2. What is Dart?
Dart is a programming language optimized for building mobile, desktop, server, and web applications. Dart is the programming language used by Flutter for building UIs.
...
The widget tree in Flutter is a hierarchy of widgets that describes the structure of the UI. Every Flutter application consists of widgets, which are nested inside each other to create the UI elements.
...
BuildContext is the handle to the location of a widget in the widget tree. It allows widgets to access their position in the tree and to interact with their parents or children.
8. How do you manage state in Flutter?
 Common methods of state management in Flutter include:
o	setState()
o	Provider
...
Keys are used in Flutter to preserve the state of widgets across widget rebuilds. They help Flutter differentiate between widgets that have the same type and rebuild efficiently.
11. What is an InheritedWidget?
InheritedWidget is a base class in Flutter used to pass data down the widget tree. It is often used for sharing state across multiple child widgets without needing to pass the state explicitly through constructors.

12. What is a Future and FutureBuilder in Flutter?
...
13. What is a Stream and StreamBuilder in Flutter?
...
14. What is the difference between async and await in Dart?
...
15. How does navigation work in Flutter?
 Flutter uses the Navigator widget for navigation and routing. It maintains a stack of routes (screens), and you can push a new route onto the stack or pop an existing route off it.
o  Navigator.push() to navigate to a new screen.
o  Navigator.pop() to return to the previous screen.
16. Explain ListView.builder in Flutter.
 ListView.builder is a constructor that builds a scrollable list of widgets lazily. It only renders the visible items and builds widgets on demand as they are scrolled into view, making it memory efficient.
17. What is the difference between mainAxisAlignment and crossAxisAlignment?
These properties are used in the Row and Column widgets.
...
18. What is the role of pubspec.yaml in Flutter?
pubspec.yaml is a configuration file in Flutter where you define the app's dependencies (e.g., packages, plugins), environment details, assets (images, fonts), and versioning information.
19. How do you handle exceptions in Dart?
Exceptions in Dart are handled using try-catch blocks:
...
20. What are Flutter plugins and packages?
...
21. What is the setState() method in Flutter?
setState() is used to notify the Flutter framework that the state of the widget has changed, and it should be rebuilt with the new data.

22. Explain how you would optimize the performance of a Flutter app.
...
23. What is the role of Isolate in Flutter?
Isolate is a mechanism to run Dart code in parallel. It allows the execution of code in a separate thread, independent of the main thread, which is useful for heavy computations to avoid blocking the UI thread.
24. What are Zones in Dart?
A Zone in Dart is an execution context that provides a way to handle asynchronous code (like timers, Future, etc.). Zones can be used to track asynchronous errors or to run code within a particular context.
25. Explain the Bloc pattern in Flutter.
Bloc (Business Logic Component) is an architectural pattern for managing state in Flutter. It separates business logic from the UI, with Streams handling inputs (events) and outputs (states). It is highly scalable and testable.
26. What is the difference between StatefulWidget and InheritedWidget?
...
27. What is Provider in Flutter?
Provider is a wrapper around InheritedWidget and is used for managing state across the widget tree efficiently. It is one of the most commonly used state management solutions in Flutter.
28. What is the dispose() method in Flutter?
dispose() is called when a StatefulWidget is removed from the widget tree permanently. It is used to clean up resources like StreamControllers, TextEditingControllers, or other listeners.
29. Explain widget testing in Flutter.
Widget testing is done using the flutter_test package, allowing you to test the UI and interactions of your Flutter app. It involves testing individual widgets by simulating user interaction and verifying the output.
30. How does Flutter render widgets?
Flutter uses a layered architecture to render widgets. It involves multiple layers like the Framework layer, Engine layer, and Platform layer. Widgets are first rendered as part of the widget tree, which then transforms into the render tree and painted on the screen by the engine.
31. Explain the difference between abstract class and interface in Dart.
Dart doesn’t have a separate interface keyword. Every class in Dart is implicitly an interface, and other classes can implement it. An abstract class cannot be instantiated but can be extended or implemented by other classes
32. What is the difference between hot reload and hot restart in Flutter?
...
33. How do you handle async operations in Flutter?
Async operations in Flutter are handled using Future and async/await. You use Future to represent potential values that will be available in the future, and async/await keywords to write asynchronous code that looks synchronous.

Future<String> fetchData() async {
  var response = await http.get(Uri.parse('https://example.com'));
  return response.body;
}
34. What is the difference between StatelessWidget and StatefulWidget?
...
35. What are some common state management solutions in Flutter?
Some common state management solutions in Flutter are:
...
36. What are Flutter’s constraints, sizes, and alignments?
...
37. How do you handle navigation between screens in Flutter?
Flutter uses the Navigator class for navigation.
...
38. What is a FutureBuilder in Flutter?
FutureBuilder is a widget that allows you to build a UI based on the result of a Future. It rebuilds itself when the Future is completed.
...
39. How would you optimize a Flutter app?
To optimize a Flutter app:
...
40. Explain how you would fetch and display data from an API in Flutter.
You would use the http package to fetch data and display it using FutureBuilder.
...
To manage different screen sizes, you can use MediaQuery to get the screen’s width and height and design the UI accordingly.
...
You can implement dark mode by using the ThemeData class and conditionally switching themes based on user preference.
...
You can handle form validation using Form and TextFormField widgets with validators.
...
Gestures in Flutter can be handled using the GestureDetector widget. You can detect various user interactions such as tap, double-tap, long press, drag, etc.
...
Errors in Flutter can be handled using try-catch blocks in Dart for catching exceptions.
...
The main() function is the entry point of every Flutter app. It is where the app execution starts.
void main() {
  runApp(MyApp());
}
...
Flutter provides AnimationController and AnimatedBuilder to create animations. You can also use pre-built animations like AnimatedOpacity, AnimatedContainer, etc.
...
Flutter is an open-source UI toolkit developed by Google for building natively compiled applications for mobile, web, and desktop from a single codebase. It uses Dart as the programming language and allows developers to create highly performant, beautiful apps with a reactive framework similar to React.
51. What is a Ticker in Flutter?
A Ticker is a class that triggers a callback on every frame, making it useful for animations. It is typically used with an AnimationController to create animations.
...
Mixins are a way to reuse a class's code in multiple class hierarchies. They allow you to add functionality to a class without using inheritance. In Flutter, mixins are often used to implement behavior in classes like StatefulWidget.
mixin LoggerMixin {
...
An isolate is a way to run code in a separate memory space, allowing you to perform heavy computations without blocking the main thread. Each isolate has its own memory and event loop.
...
The async keyword is used to mark a function as asynchronous, meaning it will return a Future. The await keyword is used to pause the execution of the function until the awaited Future completes. This allows for non-blocking code execution.
...
Extension methods allow you to add new functionality to existing classes without modifying them. This can be useful for creating utility methods that operate on the class.
...
A Singleton pattern ensures that a class has only one instance and provides a global access point to that instance.
...
Dependency management in Flutter can be handled using various methods, including:
...
Listenable is an interface that can be used to notify listeners about changes. Commonly used with ChangeNotifier to implement state management.
...
WidgetsBinding is the glue between the Flutter framework and the Flutter engine. It allows you to schedule frame callbacks and access various services, like media queries and accessibility features.
64. Explain the GlobalKey in Flutter.
A GlobalKey allows you to access the state of a widget from anywhere in the widget tree, which is especially useful for managing state between different parts of your app.
...
Testing in Flutter can be done at three levels:
